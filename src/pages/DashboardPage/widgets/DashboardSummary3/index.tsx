...
class DashboardSummary extends Component<Props> {
  ...
  async getStakeInfo(): Promise<any> {
    ...
    stake_info = {
      ...
      Staking2_30d_min_time_withdraw: Number(await instance.Staking2_30d_min_time_withdraw()) * 1000,
  ...
  render(): ReactNode {
    ...
    let time_withdraw = stake_info?.timeFirstStake + stake_info?.Staking2_30d_min_time_withdraw
          ...
          <Grid item {...gridItemPros}>
            <BoxOutlineSecondary>
              <Box className='content'>
                <Text>MIN</Text>
                <Text variant='h3'>{BNFormat(stake_info?.Staking2_min / 1e18)} {infos?.token?.symbol}</Text>
                <Text>MAX</Text>
